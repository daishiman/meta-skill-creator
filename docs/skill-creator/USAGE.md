...
要件の構造化（interview-result.json）
    ↓
[分岐] マルチスキルの場合:
  Phase 0.9: マルチスキル設計 → 各サブスキルに対して以下を繰り返し
    ↓
最適なリソースを自動選択
    ↓
...
## インタビュー深度の選択（interviewDepth）

インタビュー開始時に、質問量を3段階から選択できます。

```
Claude: インタビューの深さを選択してください：
        1. quick - 最小限の質問（3-4問）+ 残りは自動推定
        2. standard - 通常の8段階インタビュー（デフォルト）
        3. detailed - 網羅的なヒアリング（10-15問）
```

| 深度 | 質問数 | 特徴 | 推奨ケース |
|------|--------|------|-----------|
| **quick** | 3-4問 | 最小限の質問で残りは自動推定 | 要件が明確な場合、プロトタイプ作成 |
| **standard** | 8問 | 通常の8段階インタビュー | 一般的なスキル作成（デフォルト） |
| **detailed** | 10-15問 | 網羅的に全項目をヒアリング | 複雑なスキル、外部連携が多い場合 |
...
2. **マルチスキル設計**（該当する場合）: `multi-skill-graph.json`に保存
3. **リソース選択**: 要件に基づき最適なリソースを自動選定
4. **分析・設計**: 要件を分析し、スキル構造を設計
5. **依存関係解決**（該当する場合）: `skill-dependency-graph.json`に保存
6. **生成**: SKILL.md、スクリプト、エージェントを生成
7. **外部CLI委譲**（該当する場合）: Gemini CLI等で一部サブタスクを実行
8. **検証**: バリデーションを実行
...
インタビューで複数スキルの同時作成が必要と判断された場合に実行されます。

```
Claude: 複数のスキルに分割して設計します。
        依存関係グラフ：
        skill-a → skill-b → skill-c
        作成順序: skill-a → skill-b → skill-c
```

**生成される成果物**: `multi-skill-graph.json`

---

## クロススキル依存解決（Phase 2.5）

他のスキルに依存する場合に実行されます。

```
Claude: 以下のスキルへの依存を検出しました：
        - skills/other-skill/scripts/utils.js（ユーティリティ関数）
        相対パスで参照を設定しますか？
```

**生成される成果物**: `skill-dependency-graph.json`

---

## 外部CLIエージェント委譲（Phase 4.5）

一部のサブタスクを外部CLIツール（Gemini CLI等）に委譲する場合に実行されます。

```
Claude: 以下のサブタスクをGemini CLIに委譲します：
        - テストコードの生成
        実行してよろしいですか？
```

**生成される成果物**: `external-cli-result.json`
...
Claude Code / Codex / Gemini を使い分けて最適な実行エンジンを選択するモードです。
