...
| データ処理     | parser, transformer, aggregator | type-parser.md, type-transformer.md |
...
| 2        | 抽象度レベルから基本エージェントを選択         | LLM  |
| 3        | 機能リストからスクリプトタイプ・アセットを選択 | LLM  |
| 4        | 外部連携からリファレンスを選択                 | LLM  |
| 5        | 構成タイプから基本アセットを選択               | LLM  |
| 6        | スクリプト有無からスキーマ・スクリプトを追加   | LLM  |
| 7        | オーケストレーション要件からリソースを追加     | LLM  |
| 8        | ドキュメント要件からリソースを追加             | LLM  |
| 9        | 必須リソース（共通）を追加                     | LLM  |
| 10       | 依存関係を解決し順序付け                       | LLM  |
| 11       | resource-selection.jsonを出力                  | LLM  |
